...
Taxi and ride hailing operators need to understand when and where revenue is generated in order to allocate drivers efficiently and maximise fleet productivity. Raw trip data alone is not enough, it needs to be cleaned, enriched with geographic context, and aggregated to reveal actionable patterns.
...
The average tip rate across the cleaned dataset provides a baseline for understanding payment preferences by zone. Zones with below- verage tip rates may indicate cash payment dominance or lower customer satisfaction, both worth investigating at a zone level.
